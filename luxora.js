...
document.addEventListener("DOMContentLoaded", () => {
  ...
  /* ==================== HOTEL IMAGE SLIDER ==================== */
  const hotelSlideImg = document.getElementById("slideImg");
  const hotelNextBtn = document.getElementById("NextBtn");
  const hotelPrevBtn = document.getElementById("PrevBtn");

  if (hotelSlideImg && hotelNextBtn && hotelPrevBtn) {
    const images = ["photo2.jpg","photo3.jpg","photo4.jpg"];
    let currentIndex = 0;

    const updateImage = () => hotelSlideImg.src = images[currentIndex];
    ...
    hotelPrevBtn.addEventListener("click", () => {
  ...
  const eventsSlidesImg = document.getElementById("eventsSlidesImg");
  const eventsNext = document.getElementById("eventsNextBtn");
  const eventsPrev = document.getElementById("eventsPrevBtn");
  const eventsDotsContainer = document.getElementById("eventsDots");

  if (eventsSlidesImg && eventsNext && eventsPrev && eventsDotsContainer) {
    const images = [
      "photo6.jpg",
      "photo7.jpg",
      "photo8.jpg",
    ...
    function updateSlide() {
      eventsSlidesImg.src = images[currentIndex];
  ...
  const roomSliderImg = document.getElementById("roomSliderImg");
  const nextBtn = document.getElementById("nextBtn");
  const prevBtn = document.getElementById("prevBtn");
  const roomDots = document.getElementById("roomDots");

  if (slidder && roomSliderImg && nextBtn && prevBtn && roomDots) {
    const images = slidder.dataset.images
      .split(",")
      .map(img => img.trim());
    ...
    const dots = roomDots.querySelectorAll(".dot");

    function updateSlide() {
      roomSliderImg.src = images[currentIndex];
      dots.forEach(dot => dot.classList.remove("active"));
      dots[currentIndex].classList.add("active");
    }
